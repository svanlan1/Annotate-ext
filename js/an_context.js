...
function add_marker_select_options(divItem, el, selVal, prev, prevObj) {
	...
	var d = $('<div />').addClass('marker marker_recommendation').attr('id', 'marker_select_text_div_' + count).appendTo(divItem);
	...
	$(options).each(function(i,v) {
		if(localStorage.getItem('set') === "custom") {
			$('<option />').attr('value', v.name).attr('data-marker-rec', v.rec).attr('data-marker-example', v.ex).text(v.name).appendTo(sel);
		} else {
